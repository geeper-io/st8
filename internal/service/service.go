...
func (s *Service) Apply(ctx context.Context, input ApplyInput) (*ApplyResult, error) {
	...
	if err := checkScope(ctx, input.Scope.Namespace, input.Scope.Branch); err != nil {
		return nil, err
	}
...
func (s *Service) DeleteCheckpoint(ctx context.Context, ns, name string) error {
	...
	if err := checkScope(ctx, ns, ""); err != nil {
		return err
	}
	return s.engine.DeleteCheckpoint(ctx, ns, name)
}
